<?php
// $Id: subscribers.php 1026 2008-04-14 15:27:26Z marcan $
//  ------------------------------------------------------------------------ //
...
switch ($op) {
    default:
    case "list":
    	// retreive the Newsletter object to determine the newsletter_type
    	$newsletterObj = $smartmail_newsletter_handler->get($id);

        $criteria = new CriteriaCompo(new Criteria('newsletterid', $id));
        $count = $handler->getCount($criteria);
        $start = isset($_REQUEST['start']) ? intval($_REQUEST['start']) : 0;
        $limit = 50;
        $subscribers =& $handler->getRecipientList($id, $newsletterObj->getVar('newsletter_type'), $limit, $start);
        unset($criteria);
		...
		/**
		 * TODO : adding subscribers in admin has been disabled as XoopsFormSelectUser
		 * crashes the system when MANY users...
		 * We will need to better handle this.
		 */
        /*
