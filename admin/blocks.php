<?php
// $Id: blocks.php 1026 2008-04-14 15:27:26Z marcan $
//  ------------------------------------------------------------------------ //
...
$op = isset($_REQUEST['op']) ? $_REQUEST['op'] : "blocks";
...
switch ($op) {
    ...
    case "block":
        ...
        if (isset($_REQUEST['nb'])) {
            ...
            if (!is_object($block) ) {
                redirect_header("javascript: history.back(-1)");
            }
    ...
    case "block_save":
        ...
        if ($newsletterblock_handler->insert($newsletterblock)) {
            ...
            redirect_header("blocks.php?id=".$id, 2, sprintf(_NL_AM_SAVEDSUCCESS, "Block"));
        }
    ...
    case "block_delete":
        ...
        if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
            if ($newsletterblock_handler->delete($obj)) {
                redirect_header('blocks.php?id='.intval($obj->getVar('newsletterid')), 3, sprintf(_NL_AM_DELETEDSUCCESS, _NL_AM_BLOCK));
            }
            else {
