<?php
// $Id: subscription.php 1026 2008-04-14 15:27:26Z marcan $
//  ------------------------------------------------------------------------ //
...
function smartmail_subscriptionForm() {
	...
	if (is_object($xoopsUser)) {
		...
		/**
		 * Remove newsletter that would not be in $allowedNewsletters
		 * This could happen when a newsletter was once with a recipients
		 * type = 1 (users who subscribe to this newsletter) and some users did
		 * subscribe to it and now this nesletter has a recipient type > 1 (users
		 * with user_mailok=1 OR All users)
		 */
		$updated_subcribedNewsletters = array();
		foreach($subcribedNewsletters as $k => $v) {
			if (isset($allowedNewsletters[$k])) {
				$updated_subcribedNewsletters[$k] = $v;
			}
		}
		$subcribedNewsletters = $updated_subcribedNewsletters;
		...
		$updated_subcribedNewsletters = array();
		foreach($allowedNewsletters as $k => $v) {
			$updated_subcribedNewsletters[$k] = $v->toArray();
		}
		$xoopsTpl->assign('newsletters', $updated_subcribedNewsletters);
	}
